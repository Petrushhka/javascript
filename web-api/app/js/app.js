...
import {
  ...
  const calculate = (type) =>{ 
    ...
    //type 에 들어있는 기호에 따라 연산을 진행
    //연산의 결과를 두번재 section에 렌더링
    // 다음 연산을 위해서 입력창을 비워주면 좋겠다.
    

  }

  document.getElementById('calc-actions')
        .addEventListener('click',(e)=>{
            if(!e.target.matches('button')) return;
            calculate(e.target.textContent);
  });

  //eventHandler 등록자리에 함수호출문을 작성하면 함수의 호출 결과를 이벤트로 등록하겠다고 인식해요.
